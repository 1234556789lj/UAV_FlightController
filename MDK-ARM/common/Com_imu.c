 ...
 * @return {*}
 */
void Common_IMU_GetEulerAngle(Gyro_Acc_Data *gyroAccel,
                              Euler_Data *eulerAngle,
                              float dt)
{
    volatile struct V
    {
    ...
    // 加速度归一化
    NormQuat = Q_rsqrt(squa(gyroAccel->acc.acc_x) +
                       squa(gyroAccel->acc.acc_y) +
                       squa(gyroAccel->acc.acc_z));

    Acc.x = gyroAccel->acc.acc_x * NormQuat;
    Acc.y = gyroAccel->acc.acc_y * NormQuat;
    Acc.z = gyroAccel->acc.acc_z * NormQuat;
    // 向量差乘得出的值
    ...
    normAccz = gyroAccel->acc.acc_x * vecxZ + gyroAccel->acc.acc_y * vecyZ + gyroAccel->acc.acc_z * veczZ; /*Z轴垂直方向上的加速度，此值涵盖了倾斜时在Z轴角速度的向量和，不是单纯重力感应得出的值*/
}
